...
│       ├── loader.py
│       └── data/             # taxonomy, products, reviews, inspirations, locations, users, blog (.json)
```
...
- `JWT_SECRET` has no default when `APP_ENV=prod`. Startup fails if it is missing or shorter than 32 bytes (shorter HS256 keys also make PyJWT warn). The dev and Compose defaults are 32+ bytes for the same reason.
...
| `reviews` | product_id, user_id? (SET NULL; seed reviews have none), author_name, rating, comment | ck(rating 1..5), uq(product_id, user_id) |
| `users` | email (stored lowercased), password_hash, first_name, last_name | uq(email) |
| `carts` | id **UUID**, user_id? (CASCADE) | uq(user_id): one cart per user. Guest carts have no user and are reachable by ID; user carts only by token |
...
| `orders` | order_number (`FUR-` + zero-padded id, set after flush), email (lowercased lookup key), status, payment_method, billing (JSON snapshot), subtotal_minor, total_minor, user_id? (SET NULL) | uq(order_number), ix(email), ix(user_id) |
...
| `blog_categories` / `blog_posts` | slug, name, position / slug, title, excerpt, content (Markdown subset, API_CONTRACT §2.8), cover_url, author, category_id, published_at | uq(slug) / uq(slug), ix(category_id), ix(published_at) |
| `inspirations` | index_label, room, title, image_url, link, position | |
| `contact_messages` | name, email (lowercased), subject?, message | |
| `newsletter_subscribers` | email (stored lowercased) | uq(email) |

`rating_avg` and `review_count` are denormalised and updated by `catalog_service.create_review`, which keeps list queries cheap.

Every timestamp column uses `UtcDateTime` (`db/base.py`), a `DateTime(timezone=True)` that always returns aware UTC values. SQLite has no time zones, and without it the API would serialise times without the `Z`. Use it for any new datetime column.
...
- The baseline matches the design:
  ...
  - Blog categories with counts Crafts 2 / Design 8 / Handmade 7 / Interior 1 / Wood 6. The blog therefore has 24 posts, so the counts are true and pagination has 8 pages. The five newest posts are the design's titles in the design's order ("Going all-in with millennial design" … "Colorful office redesign"), published every 8 days back from 2026-09-14. Covers reuse the 8 blog images from the PDF.
- Accounts (`users.json`): `demo@furniro.test` and `empty@furniro.test`, both with password `Demo@1234`. The demo user has 2 past orders (`FUR-000001`, `FUR-000002`, which do not touch stock) and 3 liked products. Password hashes are stored precomputed, so reseeding writes identical rows. The `.test` domain (RFC 2606) is reserved; `schemas/base.py` opts it out of email-validator's special-use check so these addresses validate.
